...
public:
	...
	public:
		const static op MIN;
		const static op MAX;
		// const static op MIN = op(numeric_limits<T>::max(), [](const T &x, const T &y) {return min(x, y);}, true);
		// const static op MAX = op(numeric_limits<T>::min(), [](const T &x, const T &y) {return max(x, y);}, true);
		// const static op GCD = op(T(0), [](const T &x, const T &y) {return __gcd(x, y);}, true);
		// const static op SUM = op(T(0), [](const T &x, const T &y) {return x + y;}, false);
		// const static op XOR = op(T(0), [](const T &x, const T &y) {return x ^ y;}, false);
		// const static op AND = op(T(1), [](const T &x, const T &y) {return x & y;}, true);
		// const static op OR = op(T(0), [](const T &x, const T &y) {return x | y;}, true);
	};
...
template <typename T>
const typename sparse_table<T>::op sparse_table<T>::use::MIN = sparse_table<T>::op(numeric_limits<T>::max(), [](const T &x, const T &y) {return min(x, y);}, true);
template <typename T>
const typename sparse_table<T>::op sparse_table<T>::use::MAX = sparse_table<T>::op(numeric_limits<T>::min(), [](const T &x, const T &y) {return max(x, y);}, true);
...
void solve(int tc=1) {
	...
	sparse_table<int> S(sparse_table<int>::use::MIN), T(sparse_table<int>::use::MAX);
	S.build(b);
	T.build(b);
	...
	while(q--) {
		...
		int mn = S.query(l, r), mx = T.query(l, r);
		ld toadd = (mx - mn) / 2.0;
		if(l > 0) 
			toadd = max(toadd, (ld)T.query(0, l - 1));
		if(r + 1 < n)
			toadd = max(toadd, (ld)T.query(r + 1, n - 1));

		cout << fixed << setprecision(10) << mn + toadd << nl;
	}
}
